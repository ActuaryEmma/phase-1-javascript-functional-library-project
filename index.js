const myData = function(collection) {
  ...
  const myReduce = function(collection, callback, acc) {
    let newCollection = myData(collection);
  ...
  const mySortBy = function(arr, callback) {
    const newArr = [...arr];
    return newArr.sort(function(num1, num2) {
      if (callback(num1) > callback(num2)) {
        return 1;
      } else if (callback(num1) > callback(num2)) {
        return -1;
      } else {
        return 0;
      }
